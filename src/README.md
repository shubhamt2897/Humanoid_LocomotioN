...
Full technical documentation for `Robust Humanoid Locomotion (Unitree G1)`. See the
[top-level README](../README.md) for the project objective and results; this file is the
deep-dive for anyone building on or auditing the code itself.
...
[`rewards.py`](g1_locomotion/rewards.py); weights live in
[`config.py::RewardScales`](g1_locomotion/config.py).
...
starting-point estimates -- see [`robot.py`](g1_locomotion/robot.py).
...
- `train.py`, `play.py`, `export_policy.py` -- entry points (see "Usage" below).
...
periodic snapshot PNGs to `media/frames/` instead. `--auto_record` (with `--record`, in the normal
interactive viewer) starts capturing immediately instead of waiting for an R keypress.
...
- `-d` (detached) avoids streaming logs back to the terminal at all, which also sidesteps a local
  terminal encoding crash some consoles hit on wandb's Unicode output (the job itself keeps
  running server-side regardless; only a local log *stream* would be affected). Poll status with
  `hf jobs inspect <job_id>` and logs with `hf jobs logs <job_id>` instead of `-f` streaming.
- **Writing `--log_dir` directly onto the bucket mount is risky for long runs**: TensorBoard's
  event-file writer does frequent small appends, and that write pattern hit
  `OSError: [Errno 5] Input/output error` on the FUSE-mounted bucket partway through
  `asymmetric_payload_run_v7`, killing the job (checkpoints -- big, infrequent, atomic writes --
  had succeeded fine up to that point; only the continuous-append pattern failed). Safer pattern:
  point `--log_dir` at local container disk and run a background loop that periodically `cp`s
  checkpoints/events to the bucket mount instead of writing there continuously:
  ```
  mkdir -p /root/local_run && { \
    (while true; do sleep 300; cp -f /root/local_run/*.pt /checkpoints/<run_name>/ 2>/dev/null; \
      cp -f /root/local_run/events.out.tfevents.* /checkpoints/<run_name>/ 2>/dev/null; done) & \
    SYNC_PID=$!; \
    python train.py ... --log_dir /root/local_run --resume /checkpoints/<run_name>/model_<N>.pt; \
    kill $SYNC_PID 2>/dev/null; \
    cp -f /root/local_run/*.pt /checkpoints/<run_name>/; \
    cp -f /root/local_run/events.out.tfevents.* /checkpoints/<run_name>/; \
  }
  ```
  Reading a checkpoint from the bucket mount once at startup (`--resume`) is fine -- only the
  high-frequency append pattern is the problem. Side effect: `WandbLogWriter` names the W&B run
  after `log_dir`'s basename, so moving `--log_dir` off the bucket path also changes the run's
  display name (cosmetic only -- track the run by its W&B run ID instead).
- Same CPU-bound-stepping caveat as Phase 2/Colab above applies -- pick `t4-small` over
  ...
  across cores; the extra vCPUs otherwise sit idle. (For reference, `mujoco-warp` -- MuJoCo's own
  official GPU-batched physics backend, the real fix for Phase 2's throughput ceiling -- is a
  genuine drop-in-*API* replacement per its own docs, but getting the actual batched-tensor
  speedup means restructuring `mujoco_env.py`'s per-instance state model, not just swapping an
  import; not yet attempted here.)
